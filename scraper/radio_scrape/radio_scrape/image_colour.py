...
def dominant_colours(image_path: str, n_clusters: int = 3) -> list[str] | None:
    ...
    colours = [
        _ClusterColour(cluster_size=cluster_size, rgb=colour)
...
def _process_colour(rgb: sRGBColor) -> sRGBColor:
    ...
    def _desaturate(
        limiter: Annotated[float, Field(strict=True, gt=0, le=1)] = 0.5
    ) -> None:
        """
    ...
    def _decontrast(
        min_l: Annotated[int, Field(strict=True, gt=0, le=100)] = 12,
        max_l: Annotated[int, Field(strict=True, gt=0, le=100)] = 80,
    ) -> None:
        """
        Converts rgb to lab
        The L* value in the Lab colour space represents lightness with a range from 0 to 100.
...
def _remove_similar_colours(
    colours: list[_ClusterColour], min_delta: int = 14
) -> list[_ClusterColour]:
    """
