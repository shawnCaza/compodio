    ...
    def parse(self, response):
        ...
        for day_column in response.css("div.schedule-day"):
            for show in day_column.css("div.schedule-showWrapper"):
                ...
                current_show["showName"] = show.xpath(
                    ".//a[@class='showTitle']/text()"
                ).get()
                ...
                if (
                    ...
                    current_show["img"] = show.xpath(
                        ".//div[@class='show_image']/img/@src"
                    ).get()
                    ...
                    for p in desc_paragraphs:
                        if p.xpath("./text()") and len(
                            p.xpath("./text()").get().strip()
                        ):
                            ...
                            full_p_txt = lxml.html.fromstring(
                                p.extract()
                            ).text_content()
                    ...
                    current_show["host"] = (
                        show.xpath(".//div[@class='hosts']/text()")
                        .get()
                        .replace("hosted by", "")
                        .strip()
                    )
                    current_show["internal_link"] = show.xpath(
                        ".//div[@class='links']/a[text()='archives']/@href"
                    ).get()
                    current_show["ext_link"] = show.xpath(
                        ".//div[@class='links']/a[text()='website']/@href"
                    ).get()
                    current_show["email"] = (
                        show.xpath(".//div[@class='links']/a[text()='email']/@href")
                        .get()
                        .replace("mailto:", "")
                    )
                    current_show["source"] = "cfru"
                    current_show["duration"] = (
                        60 * 60
                    )  # all eps are broken into 1hr segments
