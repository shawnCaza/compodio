...
function LinkIcon({ icon, label, txt, link = undefined }: LinkIconProps) {
  return (
    <div className={styles.iconAndTextContainer}>
      <span className={styles.linkIcon} role='presentation'>
        {icon}
      </span>
...
export default function ShowPage() {
  ...
  const htmlDesc =
    show?.desc && show.desc.match(/^<p>/) ? show.desc : `<p>${show?.desc}</p>`
  ...
  return (
    <>
      <div className={styles.detailAndImageWrapper}>
        ...
        <div className={styles.showDetails}>
          ...
          <div className={styles.iconDetailsList}>
            ...
            {show.host && (
              <LinkIcon icon={<IoPeopleSharp />} label='Host' txt={show.host} />
            )}

            <LinkIcon
              icon={<GoGlobe />}
              label='Website'
              txt={`${show.source.toUpperCase()}`}
              link={show.internal_link}
            />
          ...
          {show.tagIds && (
            <TagsContainer
              currentTagIds={JSON.parse(show.tagIds)}
              maxTags={null}
            />
          )}
        </div>
      ...
      <ContentSection heading='Recommended' tag='h2'>
        {recShowsShuffled && (
          <CardCollection
            cardDataCollection={recShowsShuffled}
